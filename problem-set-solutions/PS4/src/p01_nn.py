...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    for x in range(input_width - conv_width + 1):
        for y in range(input_height - conv_height + 1):
            for output_channel in range(conv_channels):
                grad_wrt_weights[output_channel, :, :, :] += output_grad[output_channel, x, y] * data[:, x:(x + conv_width), y:(y + conv_height)]
                grad_wrt_data[:, x:(x + conv_width), y:(y + conv_height)] += output_grad[output_channel, x, y] * conv_W[output_channel, :, :, :]
    ...
    for x in range(0, input_width, pool_width):
        for y in range(0, input_height, pool_height):
            for input_channel in range(input_channels):
                ...
                grad[input_channel, x:(x + pool_width), y:(y + pool_height)][input_channel, np.unravel_index(np.argmax(data_), data_.shape)] = output_grad[input_channel, x // pool_width, y // pool_height]
